...
def test_login(client):
    with client:
        ...
        response3 = client.post(
            "/login",
            data={"email": "johnny@bcit.ca", "password": "Acit2911!fun"},
        ...
        # login
        client.post(
            "/login", data={"email": "test@test.com", "password": "Aa12345678!"}
        )
...
def test_recipe_delete_get(client):
    ...
    with client:
        ...
        data = client.get("/recipe/view/3/delete", follow_redirects=True).data.decode(
            "utf-8"
        )
...
def test_recipe_update_post(client):
    ...
    with client:
        ...
        response = client.post(
            "/recipe/view/1/update", data=NEW_RECIPE, follow_redirects=True
        )
        assert response.status_code == 200
        assert response.request.path == "/profile"
...
def test_register_user(client):
    ...
    with client:
        ...
        response = client.post(
            ...
            data={
                "email": "alice@example.com",
                "password": "Acit2911!fun",
                "confirm_password": "acit",
            },
            follow_redirects=True,
        )
        assert response.status_code == 200
        ...
        response = client.post(
            "/register",
            data={
                "email": "johnny@bcit.ca",
                "password": "Acit2911!fun",
                "confirm_password": "Acit2911!fun",
            },
            follow_redirects=True,
        )
...
def test_filter(client):
    with client:
        ...
        data = client.post(
            "/filter", data={"meat": "Beef"}, follow_redirects=True
        ).data.decode("utf-8")
...
def test_search(client):
    with client:
        ...
        data = client.get(
            "/", data={"search": "Grilled"}, follow_redirects=True
        ).data.decode("utf-8")
        assert "Grilled Flank Steak" in data
        assert "Here are some recipes for you!" in data

        data = client.post(
            "/", data={"search": "Johnny"}, follow_redirects=True
        ).data.decode("utf-8")
        assert "Grilled Flank Steak" in data
